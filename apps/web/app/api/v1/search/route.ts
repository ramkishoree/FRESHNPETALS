...
/** Ch.12 §18 Search Experience — "Product Search, Category Search... <200ms" (blog search dropped along with the blog). Uses the GIN full-text index on products(name, description) (migration 0005) rather than ILIKE. */
...
const search = createApiRoute({
  ...
  handler: async ({ query }) => {
    ...
    if (!safeQuery) return ok({ products: [], categories: [] });
    ...
    for (const result of [productsResult, categoriesResult]) {
      if (result.error) {
